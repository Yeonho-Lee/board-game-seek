...
export function parseBoardGameData(xmlData) {
	...
	const { minPlayers: minBestPlayer, maxPlayers: maxBestPlayer } =
		parsePlayerRange(bestWith);
	...
	const rank = Array.isArray(item.statistics.ratings.ranks.rank)
		? item.statistics.ratings.ranks.rank.find((n) => n.type === "subtype")
				?.value
		: item.name?.value;
...
function getPollSummary(item) {
	...
	const bestWith = pollSummary?.result.find(
		(res) => res.name === "bestwith",
	)?.value;
	const recommendedWith = pollSummary?.result.find(
		(res) => res.name === "recommmendedwith",
	)?.value;
